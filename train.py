        ...
        for i, layer_data in enumerate(router_data_tuple):
            ...
            if hasattr(model, "dt_cross_attention") and i < len(model.dt_cross_attention):
                ...
                if hasattr(moe_module, "expert_biases"):
                    ...
                    moe_module.expert_biases.sub_(
                        update_step.to(moe_module.expert_biases.device)
                    )
    ...
    for i, d in enumerate(train_dataloader):
        ...
        if i % 100 == 0 and accelerator.is_main_process:
            ...
            if writer is not None:
                writer.add_scalar(
                    "Train/Loss", out_criterion["loss"].item(), global_step
                )
                writer.add_scalar(
                    "Train/Bpp", out_criterion["bpp_loss"].item(), global_step
                )
                writer.add_scalar("Train/Distortion", dist_losses.val, global_step)
                writer.add_scalar(
                    "Train/MoE_Imbalance_Score", moe_metrics.val, global_step
                )
...
def test_epoch(epoch, test_dataloader, model, criterion, accelerator, logger, writer):
    ...
    if criterion.loss_type == "charbonnier":
        dist_key = "char_loss"
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        default="charbonnier",
        choices=["mse", "ms-ssim", "charbonnier"],
    )
...
def main(argv):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss = test_epoch(
            epoch, valid_dataloader, net, criterion, accelerator, logger, writer
        )
