...
class DenseBlock(nn.Module):
    def __init__(self, dim=320):
        ...
        self.layer_num = 3
        self.conv_layers = nn.ModuleList([nn.Sequential(nn.GELU(), ConvWithDW(dim, dim)) for _ in range(self.layer_num)])
...
class InverseHaarWaveletTransform(nn.Module):
    ...
    def forward(self, ll, hf):
        ...
        out = F.conv_transpose2d(stacked, self.filters, stride=2, groups=self.in_channels)
        return out

class SpectralMoEDictionaryCrossAttention(nn.Module):
    def __init__(self, input_dim, output_dim, mlp_rate=4, head_num=4, qkv_bias=True, 
    ...
    def forward(self, x):
        ...
        hf_att = self.process_high_freq_moe_vectorized(hf_proj) # <--- Using Optimized Func
