...
class WMSA(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = rearrange(qkv, "b n (qkv h c) -> qkv b h n c", h=self.n_heads, qkv=3).unbind(0)
        ...
        if self.type != "W":
            ...
            # Rearrange b=1 -> nW
            mask_windows = rearrange(img_mask, "b (h p1) (w p2) c -> (b h w) (p1 p2) c", p1=self.window_size, p2=self.window_size)
        ...
        # Merge Windows
        x_windows = rearrange(x_windows, "(b h w) (p1 p2) c -> b (h p1) (w p2) c", 
                              h=H // self.window_size, w=W // self.window_size, 
                              p1=self.window_size, p2=self.window_size)

        # Reverse Cyclic Shift
        if self.type == "W":
            x = torch.roll(x_windows, shifts=(self.window_size // 2, self.window_size // 2), dims=(1, 2))
        else:
            x = x_windows
...
class SpectralMoEDictionaryCrossAttention(nn.Module):
    ...
    def process_high_freq_guided(self, hf, lf):
        ...
        all_keys = self.k_high(self.ln_dict_high(self.experts_high)) # [Total_Entries, C]
    ...
    def forward(self, x):
        ...
        hf_processed = self.process_high_freq_guided(hf.permute(0, 2, 3, 1), ll_processed.permute(0, 2, 3, 1))
